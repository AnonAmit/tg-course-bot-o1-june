...
def dashboard():
    ...
    db = get_db()
    if db is None:
        flash('Database is not available.', 'danger')
        return render_template('admin_dashboard.html', title='Dashboard', stats=get_stats(), recent_payments=[], recent_logs=[])
    stats = get_stats()
...
def courses():
    ...
    if db is None:
        ...
        return render_template('courses.html', title='Courses', courses_list=[], page=1, total_pages=1, total_courses=0)
    ...
    return render_template('courses.html', title='Courses', courses=courses_list, search_query=search_query)
...
def add_course():
    ...
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot add course.', 'danger')
...
@app.route('/course/edit/<path:course_id_str>', methods=['GET', 'POST'])
@login_required
def edit_course(course_id_str):
    """Edit an existing course using MongoDB ObjectId string"""
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot edit course.', 'danger')
        return redirect(url_for('courses'))
    ...
    courses_collection = db[COLLECTIONS["courses"]]
    course_doc = courses_collection.find_one({"_id": course_oid})
    ...
    # For the template, it might be easier to work with the doc directly or convert it to a simple dict/object
    # We also need to ensure 'id' is a string if the template expects it.
    course_doc['id'] = str(course_doc['_id'])
    # Convert category_id to string for form selection, if it exists
    if course_doc.get('category_id'):
        course_doc['category_id_str'] = str(course_doc['category_id'])
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Invalid price format.', 'danger')
            return render_template('course_form.html', course=course_doc, categories=categories_list, request_form=request.form)
            
        file_link = request.form.get('file_link')
        category_id_from_form_str = request.form.get('category_id')
        category_oid_for_update = ObjectId(category_id_from_form_str) if category_id_from_form_str else None
        
        category_name_for_update = None
        if category_oid_for_update:
            cat_doc = categories_collection.find_one({"_id": category_oid_for_update})
            if cat_doc: category_name_for_update = cat_doc.get('name')

        # Get payment options as a list then join to string
        payment_options_list = request.form.getlist('payment_options')
        payment_options_str_for_update = ",".join(payment_options_list) if payment_options_list else None
        ...
        if not title or not file_link or (not is_free and price <= 0):
            flash('Please fill all required fields. Price must be > 0 unless course is free.', 'danger')
            # Pass back current form data and the course_doc for template re-population
            current_form_data = dict(request.form)
            current_form_data['is_free'] = is_free # ensure checkbox state is passed correctly
            return render_template('course_form.html', course=course_doc, categories=categories_list, request_form=current_form_data)
        ...
        update_fields = {
            ...
            "price": price,
            "file_link": file_link,
            "category_id": category_oid_for_update,
            "category_name": category_name_for_update,
            "payment_options": payment_options_str_for_update,
            "qr_code_image": qr_code_image if qr_code_image else None,
            "is_free": is_free,
            "demo_video_link": demo_video_link if demo_video_link else None,
        ...
        # Handle image upload/link
        current_image_link = course_doc.get('image_link') # Get existing image link
        image_link_from_form = request.form.get('image_link') # Link potentially pasted in form field
        image_file = request.files.get('image_upload')

        final_image_url = current_image_link # Default to not changing
        ...
        if image_file and image_file.filename and allowed_file(image_file.filename):
            ...
            flash('Invalid file type for image. Only PNG, JPG, JPEG, GIF, WEBP allowed.', 'danger')
            return render_template('course_form.html', course=course_doc, categories=categories_list, request_form=request.form)
        elif image_link_from_form:
             final_image_url = image_link_from_form # User provided a new link in the text field
        
        update_fields["image_link"] = final_image_url
        
        try:
            courses_collection.update_one({"_id": course_oid}, {"$set": update_fields})
            flash('Course updated successfully!', 'success')
            return redirect(url_for('courses'))
        except Exception as e:
            flash(f'Error updating course: {e}', 'danger')
            # Re-render form with current (failed) data and original course doc
            return render_template('course_form.html', course=course_doc, categories=categories_list, request_form=request.form)
    
    # For GET request, populate form with course_doc data
    return render_template('course_form.html', course=course_doc, categories=categories_list)

@app.route('/course/delete/<path:course_id_str>', methods=['POST'])
@login_required
def delete_course(course_id_str):
    """Delete a course using MongoDB ObjectId string"""
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot delete course.', 'danger')
        return redirect(url_for('courses'))
    ...
    payments_collection = db[COLLECTIONS["payments"]]
    payment_count = payments_collection.count_documents({"course_id": course_oid})
    if payment_count > 0:
    ...
    if result.deleted_count > 0:
        ...
        flash('Course not found or could not be deleted.', 'warning') # Should be caught by 'if not course' earlier
...
def payments():
    ...
    if db is None:
        flash('Database connection failed.', 'danger')
        return render_template('payments.html', title='Payments', payments_list=[], page=1, total_pages=1, total_payments=0)
    ...
    return render_template('payments.html', title='Payments', payments_list=payments_display_list, current_status=status_filter, search_query=search_query)
...
@app.route('/payment/approve/<path:payment_id_str>', methods=['POST'])
@login_required
def approve_payment(payment_id_str):
    """Approve a payment using MongoDB"""
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot approve payment.', 'danger')
        return redirect(url_for('payments'))
    try:
        payment_oid = ObjectId(payment_id_str)
    except Exception:
...
@app.route('/payment/reject/<path:payment_id_str>', methods=['POST'])
@login_required
def reject_payment(payment_id_str):
    """Reject a payment using MongoDB"""
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot reject payment.', 'danger')
        return redirect(url_for('payments'))
    try:
        payment_oid = ObjectId(payment_id_str)
    except Exception:
...
def users():
    ...
    if db is None:
        flash('Database connection failed.', 'danger')
        return render_template('users.html', title='Users', users_list=[], page=1, total_pages=1, total_users=0)
    ...
    users_list_cursor = users_collection.find(query_filter).sort("joined_date", -1)
    processed_users_list = []
    for user_doc in users_list_cursor:
        user_doc["id"] = str(user_doc["_id"]) # Add string id for template
        processed_users_list.append(user_doc)
        
    return render_template('users.html', title='Users', users_list=processed_users_list, search_query=search_query)
...
def ban_user(user_id_str):
    ...
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot ban user.', 'danger')
        return redirect(url_for('users'))
    try:
        user_oid = ObjectId(user_id_str)
    except Exception:
    ...
    return redirect(url_for('user_detail', user_id_str=user_id_str))
...
def unban_user(user_id_str):
    ...
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot unban user.', 'danger')
        return redirect(url_for('users'))
    try:
        user_oid = ObjectId(user_id_str)
    except Exception:
...
def logs():
    ...
    if db is None:
        flash('Database connection failed.', 'danger')
        return render_template('logs.html', title='Logs', logs_list=[], page=1, total_pages=1, total_logs=0, current_filter_type=None, current_filter_value=None)
    ...
    return render_template('logs.html', title='Logs', logs_list=logs_list, search_query=search_query,
...
@app.route('/course/<path:course_id_str>')
@login_required
def course_detail(course_id_str):
    """View course details using MongoDB ObjectId string"""
    db = get_db()
    if db is None:
        flash('Database connection failed.', 'danger')
        return redirect(url_for('courses'))
    ...
    payments_collection = db[COLLECTIONS["payments"]]
    associated_payments = list(payments_collection.find({"course_id": course_oid}))
    
    # Add user identifiers to payments for display
    users_collection = db[COLLECTIONS["users"]]
    for p_doc in associated_payments:
        if p_doc.get("user_id"):
            user = users_collection.find_one({"_id": ObjectId(p_doc["user_id"])})
            p_doc["user_identifier"] = user.get("username") or user.get("telegram_id", "Unknown User") if user else "Unknown User"
        else:
    ...
    course["id"] = str(course["_id"]) # ensure course object also has string 'id' for template if needed
...
def fix_gift_codes():
    ...
    query_filter = {
        "payment_method": "gift", 
        "details": {"$regex": r".*\*.*"}, # Corrected regex: Contains an asterisk
...
def categories():
    ...
    if db is None:
        flash('Database connection failed.', 'danger')
        return render_template('categories.html', title='Categories', categories_list=[], page=1, total_pages=1, total_categories=0)
    ...
    return render_template('categories.html', title='Categories', categories_list=categories_list_agg)
...
def add_category():
    ...
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot add category.', 'danger')
...
def edit_category(category_id_str):
    ...
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot edit category.', 'danger')
        return redirect(url_for('categories'))
    try:
...
@app.route('/category/delete/<path:category_id_str>', methods=['POST'])
@login_required
def delete_category(category_id_str):
    """Delete a category using MongoDB"""
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot delete category.', 'danger')
        return redirect(url_for('categories'))
    try:
        category_oid = ObjectId(category_id_str)
    except Exception:
...
def bot_settings():
    ...
    db = get_db()
    if db is None:
        flash('Database connection failed. Cannot load settings.', 'danger')
        # Provide form with no initial data or redirect
        form = BotSettingsForm() 
        dmca_form = DMCAPolicyForm()
        return render_template('settings.html', title='Bot Settings', form=form, dmca_form=dmca_form)
    ...
    return render_template('settings.html', title='Bot Settings', settings_dict=settings_dict)
...
def course_requests_list():
    ...
    if db is None: 
        flash('Database error.', 'danger')
        return render_template('course_requests.html', title='Course Requests', requests_list=[], page=1, total_pages=1, total_requests=0)
    ...
    return render_template('course_requests.html', title='Course Requests', requests_list=requests_display_list)
